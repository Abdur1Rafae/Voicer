...
async fn main() {
    ...
    //db_config::download_voice_notes(voice_note_collection.clone(), ObjectId::parse_str("64528120776d287f34bd08ce").unwrap()).await;
    let mut voice_notes: Vec<VoiceNote> = db_config::get_all_voice_ids_from_following(user_collection.clone(), voice_note_collection.clone(), ObjectId::parse_str("644f79f076f7ad5bde441e7a").unwrap()).await;
    db_config::sort_voice_notes_by_timestamp_desc(&mut voice_notes);
    // println!("{:?}", voices);
